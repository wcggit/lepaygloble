 ...
 */
@Configuration
public class ScanCodeOrderExcel extends AbstractExcelView {
    @Override
    protected void buildExcelDocument(Map<String, Object> map, HSSFWorkbook hssfWorkbook,
        ...
        List<ScanCodeOrder> orderList = (List<ScanCodeOrder>) map.get("scOrderList");
    ...
    public void setExcelRows(HSSFSheet excelSheet, List<ScanCodeOrder> orderList) {
        ...
        for (ScanCodeOrder order : orderList) {
            ...
            if(order.getScanCodeOrderExt()!=null && order.getScanCodeOrderExt().getPayType()!=null&&order.getScanCodeOrderExt().getPayType()==1) {
                excelRow.createCell(3).setCellValue("支付宝");
            }else {
                excelRow.createCell(3).setCellValue("微信");
            }
