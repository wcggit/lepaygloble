...
public class PartnerWalletLogService {
    ...
    public List<Map<String, Object>> list(PartnerWalletLogCriteria criteria, int pageSize) {
        String sql = " SELECT log.id logId,IFNULL(log.change_money,0) changeMoney,log.create_date changeDate,c.type_explain changeOrigin,log.order_sid orderSid,log.before_change_money beforeChange,log.after_change_money afterChange FROM partner_wallet_log log " +
    ...
    private long count(PartnerWalletLogCriteria criteria) {
        String sql = " SELECT count(*) totalElements FROM partner_wallet_log log " +
            " LEFT JOIN category c ON log.type = c.id " +
            " WHERE 1=1 ";
        if(criteria.getPartnerId()!=null) {
            sql+= " AND  log.partner_id= "+criteria.getPartnerId();
