...
//      }
//      Date end1 = new Date();
//      System.out.println(end1.getTime()-date.getTime());
//
//      Date date1 = new Date();
//      for(int i =0;i<=100;i++){
//          Query query = null;
//          if(i%2==0){
//
//              query = em.createNativeQuery("select le_jia_user.image,le_jia_user.nickname,le_jia_user.bind_date,le_jia_user.phone,ifnull(counts.total,0) as count from (select le_jia_user.id,le_jia_user.bind_merchant_date  bind_date,le_jia_user.phone_number phone,wei_xin_user.nickname nickname,wei_xin_user.head_image_url image from le_jia_user,wei_xin_user where le_jia_user.bind_merchant_id = 1 and le_jia_user.bind_merchant_date between '2016/07/01 00:00:00' and '2016/07/08 23:59:59' and le_jia_user.id = wei_xin_user.le_jia_user_id  limit 0,20) as le_jia_user left join (select sum(off_line_order_share.to_lock_merchant) as total,off_line_order.le_jia_user_id as id from off_line_order,off_line_order_share where off_line_order.id = off_line_order_share.off_line_order_id and off_line_order_share.lock_merchant_id = 1 group by off_line_order.le_jia_user_id) as counts  on   le_jia_user.id = counts.id");
//          }else{
//              query = em.createNativeQuery("select le_jia_user.image,le_jia_user.nickname,le_jia_user.bind_date,le_jia_user.phone,ifnull(counts.total,0) as count from (select le_jia_user.id,le_jia_user.bind_merchant_date  bind_date,le_jia_user.phone_number phone,wei_xin_user.nickname nickname,wei_xin_user.head_image_url image from le_jia_user,wei_xin_user where le_jia_user.bind_merchant_id = 1 and le_jia_user.bind_merchant_date between '2016/07/01 00:00:00' and '2016/07/08 23:59:59' and le_jia_user.id = wei_xin_user.le_jia_user_id  limit 0,10) as le_jia_user left join (select sum(off_line_order_share.to_lock_merchant) as total,off_line_order.le_jia_user_id as id from off_line_order,off_line_order_share where off_line_order.id = off_line_order_share.off_line_order_id and off_line_order_share.lock_merchant_id = 1 group by off_line_order.le_jia_user_id) as counts  on   le_jia_user.id = counts.id");
//
//          }
//          List<BigInteger> details = query.getResultList();
//      }
//      Date end2 = new Date();
//      System.out.println(end2.getTime()-date1.getTime());
//
//
//
//
